...
public class CliGui implements PlatformEventListener, TaskWorkflowEvent, PlatformGuiInterface {
	
protected AgentTaskDispatcher guiAgent;
	
	public CliGui(AgentTaskDispatcher agent) {
	...
	protected void setLoggingMode(String mode){
		GuiEvent event = new GuiEvent(mode, 5);
		guiAgent.postGuiEvent(event);
	}
